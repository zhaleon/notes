...
impl std::fmt::Display for EmbeddingError {
    fn fmt(&self, f: &mut std::fmt::Formatter) -> std::fmt::Result {
        match self {
            EmbeddingError::IndexError(msg) => write!(f, "Index error: {}", msg),
...
// Simple embedding function based on character frequency
...
impl EmbeddingManager {
    ...
    pub fn search(&mut self, query: &str, k: usize) -> Result<Vec<String>, EmbeddingError> {
        if self.index.is_none() {
            return Ok(vec![]);
        }
        
        let index = self.index.as_ref().unwrap();
        let embedding = generate_simple_embedding(query);
...
// Test function to verify HNSW functionality
pub fn test_hnsw() -> String {
    ...
    // Insert vectors into the index with their IDs
    hnsw.insert((&v1, 0));
    hnsw.insert((&v2, 1));
    hnsw.insert((&v3, 2));
    
    // Search for the nearest neighbor to v1
    let query = vec![0.9, 0.1, 0.0, 0.0];
    let ef_search = 50;
    let neighbors = hnsw.search(&query, 2, ef_search);
    
    // Format the results
    let mut result = String::new();
    result.push_str("HNSW Test Results:\n");
    for neighbor in neighbors {
        result.push_str(&format!("ID: {}, Distance: {}\n", neighbor.d_id, neighbor.distance));
    }
